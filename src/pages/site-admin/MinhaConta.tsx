        ...
        <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-2xl">
              Conecte ChatGPT, Claude ou Cursor ao Alpha Coach. Use o token abaixo em cada chamada para consultar treino, dieta, evolução e cadastrar alunos direto do seu assistente de IA.
            </p>
          </div>
        </div>
        {mcpToken ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              <code className="flex-1 min-w-[220px] rounded-md bg-black/40 border border-border/60 px-3 py-2 text-xs font-mono break-all">
                {showToken ? mcpToken : "•".repeat(Math.min(mcpToken.length, 36))}
              </code>
              <Button size="sm" variant="outline" onClick={() => setShowToken((s) => !s)} className="gap-1.5">
                {showToken ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />} {showToken ? "Ocultar" : "Ver"}
              </Button>
              <Button size="sm" onClick={copyToken} className="gap-1.5">
                <Copy className="h-3.5 w-3.5" /> Copiar
              </Button>
              <Button size="sm" variant="outline" onClick={rotateToken} disabled={rotating} className="gap-1.5">
                {rotating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />} Gerar novo
              </Button>
            </div>
            <div className="text-xs text-muted-foreground space-y-1">
              <p><strong className="text-foreground">URL do servidor MCP:</strong> <code className="text-primary">https://{import.meta.env.VITE_SUPABASE_PROJECT_ID}.supabase.co/functions/v1/mcp</code></p>
              <p><strong className="text-foreground">Ferramentas disponíveis:</strong> list_athletes, get_athlete_workout, get_athlete_diet, get_athlete_progress, get_athlete_anamnesis, add_athlete.</p>
              <p className="text-amber-500/90">⚠️ Mantenha este token em segredo. Ele dá acesso aos dados de todos os seus alunos.</p>
            </div>
          </div>
        ) : (
