CREATE OR REPLACE FUNCTION public.enforce_single_published_diet()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS trg_enforce_single_published_diet ON public.dietas;
CREATE TRIGGER trg_enforce_single_published_diet
AFTER INSERT OR UPDATE OF is_published ON public.dietas
FOR EACH ROW
WHEN (NEW.is_published IS TRUE)
EXECUTE FUNCTION public.enforce_single_published_diet();
...
BEGIN
    IF NOT pg_try_advisory_xact_lock(834721001) THEN
        RAISE NOTICE 'check_and_send_reminders ja em execucao; saindo.';
        RETURN;
    END IF;
