...
COMPOSITION:
- The coach from the FIRST reference image is anchored in the RIGHT side/corner of the frame, LARGE and PROMINENT (cropped from mid-thigh/waist up), arms crossed, direct gaze at camera, cinematic rim lighting. The subject must be big enough that the coach's body and shoulders extend well across toward the center and center-LEFT, filling the empty space to the left of the coach so there is no large gap on the left side of the subject — only a narrow calm strip on the far left edge for UI text.
