...
COMPOSITION:
- The coach from the FIRST reference image is anchored in the RIGHT CORNER of the frame, LARGE and PROMINENT (cropped from mid-thigh/waist up), arms crossed, direct gaze at camera, cinematic rim lighting. POSITION THE COACH LOW IN THE FRAME: the coach's body sits in the LOWER portion of the banner so it appears directly above the bottom edge of the hero (right above the UI row that reads "x/4 concluidos" just below this banner). Do NOT push the coach up to the top edge — keep the head well below the top of the frame so the coach is never hidden behind the dashboard's top text and stat cards. From that low right-corner anchor, the coach's body extends LEFTWARD across the lower-center to fill the empty lower-left space; the coach's base stays on the RIGHT, do not float the coach into the center. The coach must be fully visible IN FRONT of the dashboard's stat cards, never hidden behind them.
...
- BACKGROUND: a moody dark gym / studio with dramatic light beams, subtle haze and deep shadows.${temLogo ? `\n- The THIRD reference image is the COACH'S OWN LOGO. Render that exact logo VERY LARGE so it FILLS the empty space to the LEFT of the coach: a monumental 3D metallic brand emblem mounted on the back wall, spanning from the upper-left corner across toward the center-left, extending down behind and above the coach's shoulder. The emblem must be big enough to cover the empty area that would otherwise be bare on the left side of the frame. The coach's body partially occludes the lower-right part of the logo. Keep the logo's exact shapes, letters and proportions — do not redesign it, do not distort it, be faithful to the original logo.` : ""}
