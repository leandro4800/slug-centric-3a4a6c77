...
- BACKGROUND: a moody dark gym / studio with dramatic light beams, subtle haze and deep shadows.${temLogo ? `\n- The THIRD reference image is the COACH'S OWN LOGO (exclusive to this coach). Render that exact logo on the back wall directly BEHIND the coach, MOVED CLOSER to the coach — shifted right so it sits immediately behind the coach's head and shoulders, nearly touching them, instead of far away on the left. Keep the same size as before (do not enlarge), fully visible inside the frame with no edge cropped; the coach's body partially occludes the lower part of the logo. It is a 3D metallic brand emblem mounted on the wall. Keep the logo's exact shapes, letters and proportions — do not redesign or distort it, be faithful to the original logo. NEVER place the ALPHA COACH PRO logo on the wall or anywhere in the background — the Alpha Coach Pro emblem appears ONLY as the small crest on the coach's chest.` : ""}
