...
- BACKGROUND: a moody dark gym / studio with dramatic light beams, subtle haze and deep shadows.${temLogo ? `\n- The THIRD reference image is the COACH'S OWN LOGO. Render that exact logo LARGE on the back wall in the same upper-left/center-left position as before, but SLIGHTLY SMALLER so the ENTIRE logo is fully visible inside the frame — no edge of the logo may be cut off or cropped. It remains a 3D metallic brand emblem mounted on the back wall; the coach's body partially occludes the lower-right part of the logo. Keep the logo's exact shapes, letters and proportions — do not redesign it, do not distort it, be faithful to the original logo.` : ""}
