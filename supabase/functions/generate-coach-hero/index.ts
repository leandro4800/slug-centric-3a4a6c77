...
COMPOSITION:
- The coach from the FIRST reference image is positioned in the CENTER-RIGHT of the frame, LARGE and PROMINENT, filling most of the frame's width and height (cropped from mid-thigh/waist up), arms crossed, direct gaze at camera, cinematic rim lighting. The coach's body should occupy and dominate the central area of the banner, NOT tucked into a corner — make the subject large enough that the body extends well toward the center-left, leaving only a narrow calm strip on the far left for UI text.
...
- Only the LEFT EDGE of the frame (a narrow vertical strip) must stay visually calm and darker (negative space) so that UI text can be overlaid on top of it; the rest of the frame is filled by the coach.
